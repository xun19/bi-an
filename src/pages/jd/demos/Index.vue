...
const computePoint = (point1:any, [point2, point3]: [any, any]) => { // 计算最近点
	...
	const [x2, y2] = point2
	const [x3, y3] = point3
	const d1 = (x2 - x1) ** 2 + (y2 - y1) ** 2
	const d2 = (x3 - x1) ** 2 + (y3 - y1) ** 2
	if (d1 >= d2) {
		return point3
	}
...
const computeLeftTopPoint = (center: any[], pointOfTangency: any[], r: number) => {
	...
	const d1 = (solution_x1 - centerx) ** 2 + (solution_y1 - centery) ** 2
	const d2 = (solution_x2 - centerx) ** 2 + (solution_y2 - centery) ** 2
...
<style>
    ...
    .cursor-shoot {
        /* 需要一个判定区域？ */
        cursor: url('http://127.0.0.1:5174/src/assets/cursor/shoot.svg'), auto;
    }
    .cursor-search {
    ...
    .cursor-del {
        /* 需要细节比较明显的svg选择48的尺寸 */
        cursor: url('http://127.0.0.1:5174/src/assets/cursor/del.svg'), auto;
    }
    .cursor-del-red {
        cursor: url('http://127.0.0.1:5174/src/assets/cursor/del-red.svg'), auto;
    }
    .cursor-injection {
        /* 需要细节比较明显的svg选择48的尺寸 */
        cursor: url('http://127.0.0.1:5174/src/assets/cursor/injection.svg'), auto;
    }
    .cursor-injection-red {
        cursor: url('http://127.0.0.1:5174/src/assets/cursor/injection-red.svg'), auto;
    }
    .cursor-rescue {
        /* 需要细节比较明显的svg选择48的尺寸 */
        cursor: url('http://127.0.0.1:5174/src/assets/cursor/rescue.svg'), auto;
    }
    .cursor-rescue-red {
        cursor: url('http://127.0.0.1:5174/src/assets/cursor/rescue-red.svg'), auto;
    }
</style>
